                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-default" style="box-shadow:none">
                        <div class="panel-heading col-md-12">
                            <div class="row">
                                <div class="col-md-6">
                                    <h3 class="panel-title"><strong>Filed Reports by Response time </strong></h3>
                                </div>
                            ...
                            <div class="panel panel-default tabs" style="border: none;">
                                <div class="bs-example bs-example-tabs">
                                    <ul id="myTab" class="nav nav-tabs" role="tablist">
                                        ...
                                        <li role="presentation" class="active" style="margin-left: -5px;"><a href="#sos" id="sos-tab" role="tab" data-toggle="tab" aria-controls="sos" aria-expanded="true">Filed Reports</a></li>
                                        <li role="presentation" class=""><a href="#Popularity" role="tab" id="Popularity-tab" data-toggle="tab" aria-controls="Popularity" aria-expanded="false">SOS</a></li>
                                                ...
                                                <tbody>
                                                    ...
                                                    if (!empty($result_new)) {
                                                        ...
                                                        foreach ($result_new as $rsnew) {
                                                            foreach ($rsnew['data_new'] as $ns) {
                                                                $nskey = key($ns);
                                                                echo '<tr class="">';
                                                                echo '<td>' . $count . '</td>';
                                                                echo '<td>(' . $nskey . ': ' . $ns[$nskey] . ')' . '</td>';

                                                                echo '<td>' . (empty($State) ? $ns['GeoLocationnew'] : $State) . '</td>';

                                                                echo '<td>' . strtoupper(date('Y/d/m h:i:s a', strtotime($ns['created_at']))) . '</td>';
                                                                $from_time = $ns['created_at'];
                                                                $to_time = date('Y/d/m h:i:s a', strtotime($ns['update_at']));
                                                                $diff = strtotime($to_time) - strtotime($from_time);
                                                                echo '<td>' . strtoupper(date('Y/d/m h:i:s a', strtotime($ns['update_at']))) . '</td>';
                                            ...
                                            <table id="customers2" class="table datatable">
                                                ...
                                                <tbody>
                                                    ...
                                                    foreach ($state_new as $itm) {
                                                        $a = "current_location Like" . "'%" . $itm['LGA'] . '%' . "' AND date(SOSManagement.created_dateat) = date('" . date('Y-m-d') . "') ";
                                                        $resultnew = $this->db->get_where('SOSManagement', $a)->result_array();
                                                        if (!empty($resultnew)) {
                                                            foreach ($soscategory as $catr) {
                                                                $b = "current_location Like" . "'%" . $itm['LGA'] . '%' . "' AND SOS_category = '" . $catr['sos_category_id'] . "' AND date(SOSManagement.created_dateat) = date('" . date('Y-m-d') . "') ";
                                                                $resultnew1 = $this->db->get_where('SOSManagement', $b)->result_array();
                                                                if (!empty($resultnew1)) {
                                                    ?>
                                                                    <tr class=''>
                                                                        <td><?php echo $count; ?>.</td>
                                                                        <td><?php echo $catr['sos_category_name']; ?></td>
                                                                        <td><?php echo $itm['LGA']; ?></td>
                                                                        <td><?php echo Count($resultnew1); ?></td>
                                                                    </tr>
                                                    <?php $count++;
                                                                }
                                                            }
